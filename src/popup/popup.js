...
// v0.51.3 - Auto-embed integrity hash, auto-renewal/cancellation notice
...
class LISAPopup {
  ...
  async injectSelectedSnapshots() {
    ...
    try {
      ...
      const markdownFiles = snapshots.map(snap => {
        const md = snap.rebirthHandoff || this.convertSnapshotToMarkdown(snap);
  ...
  convertSnapshotToMarkdown(snapshot) {
    ...
    const originalLabel = snapshot.format === 'compressed' ? 'Semantic Compressed' : (snapshot.format === 'raw' ? 'Raw JSON' : 'LISA-V JSONL');
    md += '> Blocks: ' + blockCount + ' | Format: Portable Markdown | Original: ' + originalLabel + '\n\n';
  ...
  async injectSnapshotAsMarkdown(id) {
    try {
      ...
      const markdown = snapshot.rebirthHandoff || this.convertSnapshotToMarkdown(snapshot);
