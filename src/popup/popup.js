...
// v0.52.5 - Auto-embed integrity hash, auto-renewal/cancellation notice
...
class LISAPopup {
  ...
  async extractConversation() {
    ...
    try {
      ...
      const response = await this.sendMessageToTab(tab.id, { action: 'extractViaLisaV' }, 600000, 0);
  ...
  convertSnapshotToMarkdown(snapshot) {
    ...
    // ── Header — format-appropriate ──
    const isRaw = snapshot.format === 'raw' || snapshot.source === 'floating-button' || snapshot.source === 'floating-button-lisav' || (!snapshot.format && !snapshot.source?.includes('compressed'));
    let md;
    if (isRaw) {
      ...
      md += 'Use this as context for continuation or any action the user requests.\n\n';
      md += '---\n\n';
    }
